...
namespace {
    glConst GetGLDrawPrimitive(dp::MeshObject::DrawPrimitive drawPrimitive) {
        switch (drawPrimitive) {
            case dp::MeshObject::DrawPrimitive::Triangles:
                return gl_const::GLTriangles;
            case dp::MeshObject::DrawPrimitive::TriangleStrip:
                return gl_const::GLTriangleStrip;
            case dp::MeshObject::DrawPrimitive::LineStrip:
                return gl_const::GLLineStrip;
        }
        UNREACHABLE();
    }
}  // namespace
...
namespace dp {
    ...
    public:
        ...
        Build(ref_ptr <dp::GraphicsContext> context, ref_ptr <dp::GpuProgram> program) override {
            ...
            bool const isVAOSupported =
                    GLFunctions::ExtensionsList.IsSupported(
                            dp::GLExtensionsList::VertexArrayObject);
            if (isVAOSupported) {
            ...
            for (auto &buffer: m_mesh->m_buffers) {
                ...
                if (!buffer.m_data.empty()) {
                    GLFunctions::glBufferData(gl_const::GLArrayBuffer,
                                              static_cast<uint32_t>(buffer.m_data.size()) *
                ...
                if (isVAOSupported) {
                    ...
                    for (auto const &attribute: buffer.m_attributes) {
                        int8_t const attributePosition = p->GetAttributeLocation(
                                attribute.m_attributeName);
                        ASSERT_NOT_EQUAL(attributePosition, -1, ());
                        GLFunctions::glEnableVertexAttribute(attributePosition);
                        GLFunctions::glVertexAttributePointer(attributePosition,
                                                              attribute.m_componentsCount,
        ...
        DrawPrimitives(ref_ptr <dp::GraphicsContext> context, uint32_t verticesCount) override {
            ...
            GLFunctions::glDrawArrays(GetGLDrawPrimitive(m_mesh->m_drawPrimitive), 0,
                                      verticesCount);
        }
    ...
    void
    MeshObject::SetAttribute(std::string const &attributeName, uint32_t bufferInd, uint32_t offset,
    ...
    void
    MeshObject::Build(ref_ptr <dp::GraphicsContext> context, ref_ptr <dp::GpuProgram> program) {
        Reset();
    ...
    std::vector<float> MeshObject::GenerateNormalsForTriangles(std::vector<float> const &vertices,
        ...
        for (size_t triangle = 0; triangle < trianglesCount; ++triangle) {
            ...
            for (size_t vertex = 0; vertex < 3; ++vertex) {
                ...
                v[vertex] = glsl::vec3(vertices[offset], vertices[offset + 1],
                                       vertices[offset + 2]);
            }

            glsl::vec3 normal = glsl::cross(
                    glsl::vec3(v[1].x - v[0].x, v[1].y - v[0].y, v[1].z - v[0].z),
